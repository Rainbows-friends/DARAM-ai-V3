...
def train_face_detection_model(use_dropout):
    ...
    if use_dropout:
        model.classifier[6] = nn.Sequential(nn.Dropout(0.5), nn.Linear(model.classifier[6].in_features, 2))
    else:
...
if __name__ == "__main__":
    if os.path.exists('face_detection_metrics.xlsx'):
        os.remove('face_detection_metrics.xlsx')
    ...
    if os.path.exists('face_recognition_model.pth'):
        os.remove('face_recognition_model.pth')
    if os.path.exists('face_detection_metrics.png'):
        os.remove('face_detection_metrics.png')
    ...
    train_face_detection_model(use_dropout)
